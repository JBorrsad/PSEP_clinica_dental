...
namespace API.Services
{
    ...
    public class AppointmentNotificationService
    {
        ...
        // Lista para clientes WebSocket (para el panel de administración)
        private static readonly ConcurrentDictionary<string, WebSocket> _webSocketClients 
        ...
        // Métodos para gestionar clientes WebSocket
        public static void AddWebSocketClient(WebSocket webSocket)
        {
            string clientId = Guid.NewGuid().ToString();
            _webSocketClients[clientId] = webSocket;
            Console.WriteLine($"WebSocket client connected. Assigned ID: {clientId}");
        }
        
        public static void RemoveWebSocketClient(WebSocket webSocket)
        {
            var clientToRemove = _webSocketClients.FirstOrDefault(c => c.Value == webSocket);
            if (clientToRemove.Key != null)
            {
                _webSocketClients.TryRemove(clientToRemove.Key, out _);
                Console.WriteLine($"WebSocket client {clientToRemove.Key} disconnected.");
            }
        }
        ...
        private static async Task NotifyAllClientsAsync(string action, object data)
        {
            ...
            // Notificar a los clientes TCP (aplicaciones de escritorio)
            await NotifyTcpClientsAsync(messageBytes);
            
            // Notificar a los clientes WebSocket (navegador)
            await NotifyWebSocketClientsAsync(notification);
        }
        
        private static async Task NotifyTcpClientsAsync(byte[] messageBytes)
        {
            ...
            foreach (var clientPair in _connectedClients)
            {
                ...
                {
                    Console.WriteLine($"Error sending to TCP client {clientPair.Key}: {ex.Message}");
                    deadClients.Add(clientPair.Key);
                }
            }
        ...
        private static async Task NotifyWebSocketClientsAsync(AppointmentNotification notification)
        {
            var deadClients = new List<string>();
            var messageBytes = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(notification));
            
            foreach (var clientPair in _webSocketClients)
            {
                try
                {
                    if (clientPair.Value.State == WebSocketState.Open)
                    {
                        await clientPair.Value.SendAsync(
                ...
                {
                    Console.WriteLine($"Error sending to WebSocket client {clientPair.Key}: {ex.Message}");
                    deadClients.Add(clientPair.Key);
                }
            }
